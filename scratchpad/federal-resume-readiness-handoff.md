...
Iteration 8 fixes a locally reproduced matcher defect: supported "work orders" and "work-orders" were treated as different qualifications. Supported handler cases improved from 4/16 to 16/16, with candidate bytes preserved. Twenty-six negative cases still withhold invented tools, added duties and meaning-changing short prefixes. All five local suites, actual DOCX rendering and Netlify packaging pass. See [iteration 8](federal-resume-iteration-8/readiness.md). This is a real defect in the identified check, but it is not proven to be the exact cause of the historical withheld draft.

Dean's next step: push codex/federal-resume-readiness to refresh PR #59, keeping base ops/openai-parallel-clone. PREVIEW WARRANTED: verify a fresh immutable deployment before one bounded federal acceptance attempt. Do not repeat the failed 9f39034 run. Hosted federal acceptance, actual federal artifact, phone/manual accessibility and production release remain pending. Generation instructions, civilian format/export, release invariants, privacy and spending controls are preserved. No agent push, main merge or skill addition; cache remains v152 because no precached asset changed.
