...
const open=source.match(/React.createElement\(GuardCareerSupport, \{ colors: C, target: gapState.draft.target, openPlan: function\(office\) \{ ([^}]+) \}/)[1];const route={};vm.runInNewContext(open,{setGuardOfficeOffer:v=>route.offer=v,setPathwayMode:v=>route.mode=v,setPathwayStep:v=>route.step=v,setActiveTab:v=>route.tab=v,office:null,window:{scrollTo(){}}});assert.deepEqual(route,{offer:null,mode:"planner",step:1,tab:"pathway"});
